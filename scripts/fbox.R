...
# ASN BSP
PWMs = list()
lines = readLines(file("inst/alignments/F-box_bacteria.seq.fa",open="r"));
PWMs[['bacteria']]=getPwmFromAlignment(lines[grep("^[^>]",lines)],alphabet=ASN,pseudoCount=0);
PWMs[['bacteria']][,48]=1/20
lines = readLines(file("inst/alignments/F-box_fungi.seq.fa",open="r"));
PWMs[['fungi']]=getPwmFromAlignment(lines[grep("^[^>]",lines)],alphabet=ASN,pseudoCount=0);
lines = readLines(file("inst/alignments/F-box_metazoa.seq.fa",open="r"));
PWMs[['metazoa']]=getPwmFromAlignment(lines[grep("^[^>]",lines)],alphabet=ASN,pseudoCount=0);
lines = readLines(file("inst/alignments/F-box_viridiplantae.seq.fa",open="r"));
