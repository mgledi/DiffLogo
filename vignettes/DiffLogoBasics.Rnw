...
pwm_long <- list$"Hsapiens-JASPAR_CORE-CTCF-MA0139.1"[4:1,] ## reverse complement
...
for (name in motif_names_asn) {
  ...
  file = system.file(fileName, package = "DiffLogo")
  motifs_asn[[name]] = getPwmFromFastaFile(file)
}
@
...
User is able to get aligned PWMs:

## align pwms and than plot seqLogo of them
pwms = list(pwm1_long, pwm1, pwm2)
multiple_pwms_alignment = multipleLocalPwmsAlignment(pwms)
aligned_pwms = extendPwmsFromAlignmentVector(multiple_pwms_alignment$alignment$vector, pwms)
seqLogo(aligned_pwms[[2]]) ## seqLogo of aligned pwm1
