...
PWMs can be represented as object of type pwm, data.frame, or matrix. In the package DiffLogo, there are example PWMs in folder \emph{pwm} as tab--delimited files (extracted from [3]). 
